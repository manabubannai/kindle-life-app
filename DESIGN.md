...
| 論点 | 決定 | 理由 |
|---|---|---|
| 配信単位 | **1記事 = 1通 = Kindleの1冊（個別配信）**。例外として差出人単位の**週1まとめ**を任意指定できる（v0.3） | v0.1は2日に1回の合本ダイジェストだったが、1件ずつ届く方が使いやすいためv0.2で転換。書名が記事タイトルになり、読みたいものだけ開ける。毎日届く高頻度メルマガは1通ずつだと多すぎるため、v0.3で「差出人の右にタイトルを書いたものだけ週1（月曜朝）にまとめて1冊」を追加 |
...
- **赤枠② `NEWSLETTER_LIST`**（B13:B42・`ISEMAIL`検証。書いた行が有効、解除はセルを空に）
  - 右隣C列 `NEWSLETTER_DIGEST_TITLES`（任意）: タイトルを書いた差出人だけ週1まとめ配信。タイトルがそのまま書名の元になる
- **赤枠③ `BLOG_LIST`**（E13:E42・`^https?://`検証。トップページURLでOK＝フィード自動検出。右隣のF列にスクリプトがブログ名/エラーを自動表示）
...
### 週1まとめ（v0.3・差出人単位の任意オプション）
- 毎時実行でまとめ指定差出人の新着を検知したら、**送らずにメールIDだけ** `weeklyPending[タイトル]` に貯めて処理済みに記録（本文はScriptPropertiesの容量制約で貯めない。送信時に `GmailApp.getMessageById` で引き直す）
- 送りどき: 月曜6時以降の最初の実行（同日1回まで）。実行停止で月曜を逃した場合は最古の未送信が9日を超えた時点で曜日を待たず送る救済つき
- 1タイトル = 1冊（目次＋各記事`<h1>`章＋`<hr>`区切り）。書名 = 「タイトル 7/27〜8/2」。画像予算は1冊ごとにリセット
- pendingの削除は1冊送るたびにstateへ反映 → 途中失敗しても送信済み分は再送されず、未送信タイトルだけ次の実行で再試行。シートから差出人を消しても貯まった分は送り切る
